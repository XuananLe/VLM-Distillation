...
# Loss = ce_loss + alpha * kd_loss + policy_alpha * reinforce_policy_loss
ALPHA="${ALPHA:-0.5}"
TEACHER_WEIGHTING_STRATEGY="reinforced_selection"
...
NUM_TEACHERS="${#TEACHER_MODEL_IDS[@]}"
...
if [[ "${DISTILLATION_LOSS}" == "trie_wasserstein_loss" ]]; then
    RUN_TAG="${RUN_TAG:-reinforce_${DATASET_NAME}_${DISTILLATION_LOSS}_rho${TRIE_WASSERSTEIN_RHO_TAG}_topk${TRIE_WASSERSTEIN_TOPK}_alpha${ALPHA_TAG}_$(date +%Y%m%d_%H%M)}"
else
    RUN_TAG="${RUN_TAG:-reinforce_${DATASET_NAME}_${DISTILLATION_LOSS}_alpha${ALPHA_TAG}_$(date +%Y%m%d_%H%M)}"
fi
