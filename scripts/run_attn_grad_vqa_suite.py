...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Attn x Grad VQA subsets for a suite of models and rank clear outputs."
    )
...
def run_command(command: list[str], *, env: dict[str, str]) -> int:
    print("[attn-grad-suite] " + " ".join(command), flush=True)
...
def collect_suite_analysis(output_root: Path, model_statuses: list[dict[str, object]]) -> None:
    ...
    suite_analysis = {
        "selection_method": (
            ...
            "from Attn x Grad sentence heatmaps."
        ),
...
def main() -> None:
    ...
    for model_name in args.models:
        ...
        print(f"[attn-grad-suite] model={model_name} root={model_root}", flush=True)
        ...
        for dataset in args.datasets:
            ...
            if not args.no_skip_existing and summary_is_complete(summary_path, args.subset_size):
                print(f"[attn-grad-suite] skip complete {model_name} {dataset}", flush=True)
            ...
            command = [
                sys.executable,
                "demo/DEX-AR/run_attn_grad_vqa_subset.py",
                "--model-name",
                model_name,
                "--dataset",
                dataset,
    ...
    if failures:
        ...
        raise RuntimeError(f"Attn x Grad suite finished with failed runs: {formatted}")
