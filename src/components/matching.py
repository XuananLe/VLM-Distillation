...
def load_cka_json(json_path: str):
    ...
    model_a_layers = [x["layer_name"] for x in data["model_a"]["selected_layers"]]
    model_b_layers = [x["layer_name"] for x in data["model_b"]["selected_layers"]]
...
def topk_soft_match_student_teacher(
    ...
    resolved = resolve_student_teacher_similarity(payload, student_key=student_key, teacher_key=teacher_key)
    sim = resolved["sim"]
        ...
        matches.append(
            {
                ...
                "teacher_layer_indices": [int(teacher_selected_layer_indices[pos]) for pos in teacher_positions.tolist()],
