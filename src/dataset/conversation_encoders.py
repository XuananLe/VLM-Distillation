...
PROCESSOR_ENCODERS = {
    processor_type: encoder_name
    for processor_type, encoder_name in (
...
def encode_with_processor(
    ...
    encoder_name = PROCESSOR_ENCODERS.get(processor_type)
    if encoder_name is None:
        raise ValueError(f"Unsupported {role} processor type {processor_type.__name__!r}.")
    return globals()[encoder_name](sources, images, processor)
...
def encode_teacher_data(
    ...
    if isinstance(teacher_processor, dict):
        encoder_name = DICT_TEACHER_ENCODERS.get(teacher_model_id)
        if encoder_name is None:
            raise ValueError(f"Unsupported dict teacher processor for {teacher_model_id!r}.")
        teacher_data = globals()[encoder_name](sources, images, teacher_processor)
    else:
        teacher_data = encode_with_processor(sources, images, teacher_processor, role="teacher")

    return _finalize_teacher_data(teacher_data, teacher_processor, teacher_model_id, dummy_pixel_tensors)
