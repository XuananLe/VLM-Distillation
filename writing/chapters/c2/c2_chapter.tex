...
uses the specific convex-mixing form \((1-\alpha)\mathcal{L}_{CE}+\alpha\mathcal{L}_{KD}\).
...
top-\(k\) sparsification, and capacity filtering. The full framework also
supports optional balancing and expert-bias updates.
...
The high-level idea is simple. If the teacher-induced update points in a
...
In this thesis, \methodname{} uses this idea only after routing. The
GRACE-specific formulation of gradient proxies, cosine alignment, thresholding,
and fallback is presented in Chapter~\ref{chap:proposed-method}, where it is
part of the method rather than general background.
