% Cover Vietnamese 1

% Cover Vietnamese 1

% Cover english
\pagenumbering{gobble}
\begin{center}
	\begin{tikzpicture}[overlay,remember picture]
	    \draw [line width=3pt,rounded corners=0pt,
	        ]
	        ($ (current page.north west) + (25mm,-25mm) $)
	        rectangle
	        ($ (current page.south east) + (-15mm,25mm) $);
	    \draw [line width=1pt,rounded corners=0pt]
	        ($ (current page.north west) + (26.5mm,-26.5mm) $)
	        rectangle
	        ($ (current page.south east) + (-16.5mm,26.5mm) $);
	\end{tikzpicture}
	\\[1mm]
	%%12pt
	\textbf{ĐẠI HỌC QUỐC GIA HÀ NỘI\\TRƯỜNG ĐẠI HỌC CÔNG NGHỆ}\\[1cm]
	\includegraphics[width=0.2\linewidth]{figures/uet}\\[0.3cm]
	\textbf{Le Xuan An}
	\\[2cm]
	
	\large{\textbf{\shortstack[c]{\methodfull{}\\for Multimodal Understanding}}}
	\\[2.6cm]
	\normalsize{\textbf{KHOÁ LUẬN TỐT NGHIỆP
		\\[2mm]
	Ngành: Khoa học máy tính}}

    \vspace{16mm}

    \begin{center}
    \begin{tabular}{l}
    \textbf{Giảng viên hướng dẫn: Tiến sĩ Nguyễn Văn Sơn} \\
    \textbf{Giảng viên đồng hướng dẫn: Phó Giáo sư Võ Đình Hiếu}
    \end{tabular}
    \end{center}

    \vspace{8mm}

	\vfill
	\textbf{HÀ NỘI - 2026}
	\vspace{10mm}
\end{center}

\begin{center}
	
	\begin{tikzpicture}[overlay,remember picture]
	\draw [line width=3pt,rounded corners=0pt,
	]
	($ (current page.north west) + (25mm,-25mm) $)
	rectangle
	($ (current page.south east) + (-15mm,25mm) $);
	\draw [line width=1pt,rounded corners=0pt]
	($ (current page.north west) + (26.5mm,-26.5mm) $)
	rectangle
	($ (current page.south east) + (-16.5mm,26.5mm) $);
	\end{tikzpicture}
	\\[1mm]
	\textbf{ĐẠI HỌC QUỐC GIA HÀ NỘI\\TRƯỜNG ĐẠI HỌC CÔNG NGHỆ}
	\\[1cm]
	\includegraphics[width=0.2\linewidth]{figures/uet}
	\\[0.3cm]
	\textbf{Le Xuan An}
	\\[2cm]
	
	\large{\textbf{\shortstack[c]{\methodfull{}\\for Multimodal Understanding}}}
	\\[2.6cm]
	\normalsize{\textbf{KHOÁ LUẬN TỐT NGHIỆP
		\\[2mm]
	Ngành: Khoa học máy tính}}
\end{center}

\vspace{16mm}

\begin{center}
\begin{tabular}{l}
\textbf{Giảng viên hướng dẫn: Tiến sĩ Nguyễn Văn Sơn} \\
\textbf{Giảng viên đồng hướng dẫn: Phó Giáo sư Võ Đình Hiếu}
\end{tabular}
\end{center}

\vspace{8mm}

\vfill
\begin{center}
	\textbf{HÀ NỘI - 2026}
	\vspace{10mm}
\end{center}


\begin{center}
	
	\begin{tikzpicture}[overlay,remember picture]
	\draw [line width=3pt,rounded corners=0pt,
	]
	($ (current page.north west) + (25mm,-25mm) $)
	rectangle
	($ (current page.south east) + (-15mm,25mm) $);
	\draw [line width=1pt,rounded corners=0pt]
	($ (current page.north west) + (26.5mm,-26.5mm) $)
	rectangle
	($ (current page.south east) + (-16.5mm,26.5mm) $);
	\end{tikzpicture}
	\\[1mm]
	\textbf{ VIETNAM NATIONAL UNIVERSITY, HANOI\\UNIVERSITY OF ENGINEERING AND TECHNOLOGY}
	\\[1cm]
	\includegraphics[width=0.2\linewidth]{figures/uet}
	\\[0.3cm]
	\textbf{Le Xuan An}
	\\[2cm]
	
	\large{\textbf{\shortstack[c]{\methodfull{}\\for Multimodal Understanding}}}
	\\[2.6cm]
	\normalsize{\textbf{BACHELOR'S THESIS
		\\[2mm]
		Major: Computer Science}}
\end{center}

\vspace{16mm}

\begin{center}
\begin{tabular}{l}
\textbf{Advisor:} \textbf{Dr.\ Nguyen Van Son} \\
\textbf{Co-Supervisor:} \textbf{Associate Professor Vo Dinh Hieu}
\end{tabular}
\end{center}

\vspace{8mm}

\vfill
\begin{center}
	\textbf{HANOI - 2026}
	\vspace{4mm}
\end{center}
